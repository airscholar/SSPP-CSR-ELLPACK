...
__global__ void gpuMatrixVector(int rows, int *IRP, int *JA, double *AS, double *x, double *y) {
    ...
    if (row < rows) {
        int idxm = IRP[row] + tc;
        int idxn = IRP[row + 1];
        double t0 = 0.0, t1 = 0.0, t2 = 0.0, t3 = 0.0, t4 = 0.0, t5 = 0.0, t6 = 0.0, t7 = 0.0, t8 = 0.0, t9 = 0.0, t10 = 0.0, t11 = 0.0, t12 = 0.0, t13 = 0.0, t14 = 0.0, t15 = 0.0;
        for (int i = idxm; i < idxn; i += 16 * XBD) {
            t0 += AS[i] * x[JA[i]];
            if (i + XBD < idxn) {
                            ...
                            if (i + 5 * XBD < idxn) {
                                ...
                                if (i + 6 * XBD < idxn) {
                                    ...
                                    if (i + 7 * XBD < idxn) {
                                        ...
                                        if (i + 8 * XBD < idxn) {
                                            t8 += AS[i + 8 * XBD] * x[JA[i + 8 * XBD]];
                                            if (i + 9 * XBD < idxn) {
                                                t9 += AS[i + 9 * XBD] * x[JA[i + 9 * XBD]];
                                                if (i + 10 * XBD < idxn) {
                                                    t10 += AS[i + 10 * XBD] * x[JA[i + 10 * XBD]];
                                                    if (i + 11 * XBD < idxn) {
                                                        t11 += AS[i + 11 * XBD] * x[JA[i + 11 * XBD]];
                                                        if (i + 12 * XBD < idxn) {
                                                            t12 += AS[i + 12 * XBD] * x[JA[i + 12 * XBD]];
                                                            if (i + 13 * XBD < idxn) {
                                                                t13 += AS[i + 13 * XBD] * x[JA[i + 13 * XBD]];
                                                                if (i + 14 * XBD < idxn) {
                                                                    t14 += AS[i + 14 * XBD] * x[JA[i + 14 * XBD]];
                                                                    if (i + 15 * XBD < idxn) {
                                                                        t15 += AS[i + 15 * XBD] * x[JA[i + 15 * XBD]];
                                                                    }
                                                                }
                                                            }
                                                        }
        ...
        ax[tr][tc] = t0 + t1 + t2 + t3 + t4 + t5 + t6 + t7 + t8 + t9 + t10 + t11 + t12 + t13 + t14 + t15;
    }
    __syncthreads();
