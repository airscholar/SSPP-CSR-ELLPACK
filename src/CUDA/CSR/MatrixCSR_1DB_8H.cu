...
__global__ void gpuMatrixVector(int rows, int *IRP, int *JA, double *AS, double *x, double *y) {
    ...
    if (row < rows) {
        ...
        for (int i = start; i < end; i += BD * 8) {
            ...
            if (i + BD * 4 < end) sum5 += AS[i + BD * 4] * x[JA[i + BD * 4]];
            if (i + BD * 5 < end) sum6 += AS[i + BD * 5] * x[JA[i + BD * 5]];
            if (i + BD * 6 < end) sum7 += AS[i + BD * 6] * x[JA[i + BD * 6]];
            if (i + BD * 7 < end) sum8 += AS[i + BD * 7] * x[JA[i + BD * 7]];
        }
        sdata[tid] = sum1 + sum2 + sum3 + sum4 + sum5 + sum6 + sum7 + sum8;
    }
    __syncthreads();
