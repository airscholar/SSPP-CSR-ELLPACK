...
__global__ void gpuMatrixVector(int rows, int *JA, double *AS, int maxNZ, double *x, double *y) {
    ...
    if (row < rows) {
        ...
        for (int i = idxm; i < idxn; i += XBD * 16) {
            if (i < rows * maxNZ && JA[i] < rows) t1 += AS[i] * x[JA[i]];
            if (i + XBD < idxn) t2 += AS[i + XBD] * x[JA[i + XBD]];
            if (i + XBD * 2 < idxn) t3 += AS[i + XBD * 2] * x[JA[i + XBD * 2]];
            if (i + XBD * 3 < idxn) t4 += AS[i + XBD * 3] * x[JA[i + XBD * 3]];
            if (i + XBD * 4 < idxn) t5 += AS[i + XBD * 4] * x[JA[i + XBD * 4]];
            if (i + XBD * 5 < idxn) t6 += AS[i + XBD * 5] * x[JA[i + XBD * 5]];
            if (i + XBD * 6 < idxn) t7 += AS[i + XBD * 6] * x[JA[i + XBD * 6]];
            if (i + XBD * 7 < idxn) t8 += AS[i + XBD * 7] * x[JA[i + XBD * 7]];
            if (i + XBD * 8 < idxn) t9 += AS[i + XBD * 8] * x[JA[i + XBD * 8]];
            if (i + XBD * 9 < idxn) t10 += AS[i + XBD * 9] * x[JA[i + XBD * 9]];
            if (i + XBD * 10 < idxn) t11 += AS[i + XBD * 10] * x[JA[i + XBD * 10]];
            if (i + XBD * 11 < idxn) t12 += AS[i + XBD * 11] * x[JA[i + XBD * 11]];
            if (i + XBD * 12 < idxn) t13 += AS[i + XBD * 12] * x[JA[i + XBD * 12]];
            if (i + XBD * 13 < idxn) t14 += AS[i + XBD * 13] * x[JA[i + XBD * 13]];
            if (i + XBD * 14 < idxn) t15 += AS[i + XBD * 14] * x[JA[i + XBD * 14]];
            if (i + XBD * 15 < idxn) t16 += AS[i + XBD * 15] * x[JA[i + XBD * 15]];
