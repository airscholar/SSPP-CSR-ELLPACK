...
__global__ void gpuMatrixVector(int rows, int *JA, double *AS, int maxNZ, double *x, double *y) {
    ...
    if (row < rows) {
        ...
        for (int i = start; i < end; i += BD * 16) {
            ...
            if (i + BD * 8 < end && JA[row * maxNZ + i + BD * 8] != -1)
                sum += AS[row * maxNZ + i + BD * 8] * x[JA[row * maxNZ + i + BD * 8]];
            if (i + BD * 9 < end && JA[row * maxNZ + i + BD * 9] != -1)
                sum += AS[row * maxNZ + i + BD * 9] * x[JA[row * maxNZ + i + BD * 9]];
            if (i + BD * 10 < end && JA[row * maxNZ + i + BD * 10] != -1)
                sum += AS[row * maxNZ + i + BD * 10] * x[JA[row * maxNZ + i + BD * 10]];
            if (i + BD * 11 < end && JA[row * maxNZ + i + BD * 11] != -1)
                sum += AS[row * maxNZ + i + BD * 11] * x[JA[row * maxNZ + i + BD * 11]];
            if (i + BD * 12 < end && JA[row * maxNZ + i + BD * 12] != -1)
                sum += AS[row * maxNZ + i + BD * 12] * x[JA[row * maxNZ + i + BD * 12]];
            if (i + BD * 13 < end && JA[row * maxNZ + i + BD * 13] != -1)
                sum += AS[row * maxNZ + i + BD * 13] * x[JA[row * maxNZ + i + BD * 13]];
            if (i + BD * 14 < end && JA[row * maxNZ + i + BD * 14] != -1)
                sum += AS[row * maxNZ + i + BD * 14] * x[JA[row * maxNZ + i + BD * 14]];
            if (i + BD * 15 < end && JA[row * maxNZ + i + BD * 15] != -1)
                sum += AS[row * maxNZ + i + BD * 15] * x[JA[row * maxNZ + i + BD * 15]];
        }
        sdata[tid] = sum;
    }
    __syncthreads();
