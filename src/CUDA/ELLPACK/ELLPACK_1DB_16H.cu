...
__global__ void gpuMatrixVector(int rows, int *JA, double *AS, int maxNZ, double *x, double *y) {
    ...
    if (row < rows) {
        double sum = 0.0;
        for (int j = 0; j < maxNZ; j += BD * 16) {
            int index1 = row * maxNZ + j + tc;
            int index2 = row * maxNZ + j + tc + BD;
            int index3 = row * maxNZ + j + tc + BD * 2;
            int index4 = row * maxNZ + j + tc + BD * 3;
            int index5 = row * maxNZ + j + tc + BD * 4;
            int index6 = row * maxNZ + j + tc + BD * 5;
            int index7 = row * maxNZ + j + tc + BD * 6;
            int index8 = row * maxNZ + j + tc + BD * 7;
            int index9 = row * maxNZ + j + tc + BD * 8;
            int index10 = row * maxNZ + j + tc + BD * 9;
            int index11 = row * maxNZ + j + tc + BD * 10;
            int index12 = row * maxNZ + j + tc + BD * 11;
            int index13 = row * maxNZ + j + tc + BD * 12;
            int index14 = row * maxNZ + j + tc + BD * 13;
            int index15 = row * maxNZ + j + tc + BD * 14;
            int index16 = row * maxNZ + j + tc + BD * 15;
            ...
            if (j + tc + BD * 4 < maxNZ && index5 < rows * maxNZ) sum += AS[index5] * x[JA[index5]];
            if (j + tc + BD * 5 < maxNZ && index6 < rows * maxNZ) sum += AS[index6] * x[JA[index6]];
            if (j + tc + BD * 6 < maxNZ && index7 < rows * maxNZ) sum += AS[index7] * x[JA[index7]];
            if (j + tc + BD * 7 < maxNZ && index8 < rows * maxNZ) sum += AS[index8] * x[JA[index8]];
            if (j + tc + BD * 8 < maxNZ && index9 < rows * maxNZ) sum += AS[index9] * x[JA[index9]];
            if (j + tc + BD * 9 < maxNZ && index10 < rows * maxNZ) sum += AS[index10] * x[JA[index10]];
            if (j + tc + BD * 10 < maxNZ && index11 < rows * maxNZ) sum += AS[index11] * x[JA[index11]];
            if (j + tc + BD * 11 < maxNZ && index12 < rows * maxNZ) sum += AS[index12] * x[JA[index12]];
            if (j + tc + BD * 12 < maxNZ && index13 < rows * maxNZ) sum += AS[index13] * x[JA[index13]];
            if (j + tc + BD * 13 < maxNZ && index14 < rows * maxNZ) sum += AS[index14] * x[JA[index14]];
            if (j + tc + BD * 14 < maxNZ && index15 < rows * maxNZ) sum += AS[index15] * x[JA[index15]];
            if (j + tc + BD * 15 < maxNZ && index16 < rows * maxNZ) sum += AS[index16] * x[JA[index16]];
        }
        aux[tc] = sum;
    }
    __syncthreads();
