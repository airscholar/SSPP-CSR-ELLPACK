...
__global__ void gpuMatrixVector(int rows, int *JA, double *AS, int maxNZ, double *x, double *y) {
    ...
    if (row < rows) {
        ...
        for (int i = start; i < end; i += BD * 4) {
            ...
            if (i + BD < end && JA[row * maxNZ + i + BD] != -1) {
                sum += AS[row * maxNZ + i + BD] * x[JA[row * maxNZ + i + BD]];
            }
            if (i + 2 * BD < end && JA[row * maxNZ + i + 2 * BD] != -1) {
                sum += AS[row * maxNZ + i + 2 * BD] * x[JA[row * maxNZ + i + 2 * BD]];
            }
            if (i + 3 * BD < end && JA[row * maxNZ + i + 3 * BD] != -1) {
                sum += AS[row * maxNZ + i + 3 * BD] * x[JA[row * maxNZ + i + 3 * BD]];
            }
        }
        sdata[tid] = sum;
    }
