...
//    MatrixCSR_Test matrixCSR_test(M, N, nz, I, J, val);
    ...
    MatrixELLPACK matrixELLPack(M, N, nz, I, J, val, x);
    ...
    for (int tr = 0; tr < ntimes; tr++) {
        double t1 = wtime();
        unrollVResult = matrixELLPack.openMPMultiplyUnroll4V(x, y);
        double t2 = wtime();
        tmlt = dmin(tmlt, (t2 - t1));
    }
    unrollVTime = tmlt;
    ...
    for (int tr = 0; tr < M; tr++) {
        double t1 = wtime();
        unrollHResult = matrixELLPack.openMPMultiplyUnroll4H(x, y);
        double t2 = wtime();
        tmlt = dmin(tmlt, (t2 - t1));
    }
    unrollHTime = tmlt;
...
#pragma omp parallel
    {
#pragma omp master
        printf("NAME: %s, TYPE: %s, NUMPROCS: %d, SERIAL: %f, UNROLL4V_TIME: %f, UNROLL4V_GFLOPS: %f, UNROLL4V_MAX_ERROR: %f, UNROLL4H_TIME: %f, UNROLL4H_GFLOPS: %f, UNROLL4H_MAX_ERROR: %f\n",
               argv[1],
